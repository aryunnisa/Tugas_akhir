            ...
            ElevatedButton.icon(
              ...
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => const AudioDzikirPage(),
                  ),
                );
              },
              style: ElevatedButton.styleFrom(
...
class AudioDzikirPage extends StatefulWidget {
  const AudioDzikirPage({Key? key}) : super(key: key);

  @override
  State<AudioDzikirPage> createState() => _AudioDzikirPageState();
}

class _AudioDzikirPageState extends State<AudioDzikirPage> {
  final AudioPlayer _audioPlayer = AudioPlayer();
  bool isPlaying = false;

  // Ganti path audio sesuai dengan file yang kamu punya di assets
  final String audioPath = 'audio/almatsurat.MP3';
  ...
  Future<void> _seekBy(Duration offset) async {
    ...
    if (current != null && duration != null) {
      int newPosition = current.inMilliseconds + offset.inMilliseconds;
      if (newPosition < 0) newPosition = 0;
      if (newPosition > duration.inMilliseconds)
        newPosition = duration.inMilliseconds;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color.fromARGB(255, 251, 202, 143),
      appBar: AppBar(
        title: const Text('Audio Dzikir'),
        backgroundColor: const Color.fromARGB(255, 251, 202, 143),
      ),
      body: Center(
        child: Column(
          ...
          children: [
            const Icon(Icons.audiotrack, size: 100, color: Colors.brown),
            const SizedBox(height: 24),
            ElevatedButton.icon(
              icon: Icon(isPlaying ? Icons.pause : Icons.play_arrow,
                  color: const Color.fromARGB(255, 251, 202, 143)),
              label: Text(
                isPlaying ? 'Pause' : 'Play',
                style:
                    const TextStyle(color: Color.fromARGB(255, 251, 202, 143)),
              ),
            ...
            // const SizedBox(height: 16),
            // // Tombol speed 10 detik (speed control)
            // Row(
            //   mainAxisAlignment: MainAxisAlignment.center,
            //   children: [
            //     ElevatedButton(
            ...
            //       style: ElevatedButton.styleFrom(
            //         backgroundColor: Colors.brown,
            //         foregroundColor: Color.fromARGB(255, 251, 202, 143),
...
final List<DzikirItem> dzikirPagi = [
  ...
  DzikirItem(
    arabic:
        ...
        'الْخَنَّاسِۖ(٤)الَّذِيْ يُوَسْوِسُ فِيْ صُدُوْرِ النَّاسِۙ(٥)مِنَ الْجِنَّةِ وَالنَّاسِ (٦))',
    translation:
...
//
  DzikirItem(
    arabic:
        'أَصْبَحْنَا وَأَصْبَحَ  الْمُلْكُ لِلّهَِ وَالْحَمْدُ لِلّهَِ لاَ شَرِيكَ لَهُ، لاَ إِلهَ إِلَّا هُوَ وَإِلَيْهِ النُّشُوْرُ ',
    translation:
  ...
  DzikirItem(
    ...
    translation:
        'Aku rela (ridha) Allah sebagai \n Rabb-ku (untukku dan orang lain), Islam sebagai agamaku dan Muhammad صلي الله عليه وسلم sebagai Rasuk (yang diutus oleh Allah).',
  ),
  ...
  DzikirItem(
    arabic:
        "اَللَّهُمَّ صَلِّ عَلَى سَيِّدِنَا مُحَمَّدٍ عَبْدِكَ وَنَبِيِّكَ وَرَسُوْلِكَ النَّبِيِّ الأُمِّيِّ وَعَلَى آلِهِ وَصَحْبِهِ وَسَلِّمْ تَسْلِيْمًا عَدَدَ مَا أَحَاطَ بِهِ عِلْمُكَ وَخَطَّ بِهِ قَلَمُكَ وَأَحْصَاهُ كِتَابُكَ، وَارْضَ اللَّهُمَّ عَنْ سَادَاتِنَا أَبِيْ بَكْرٍ وَعُمَرَ وَعُثْمَانَ وَعَلِيْ، وَعَنِ الصَّحَابَةِ أَجْمَعِيْنَ، وَعَنِ التَّابِعِيْنَ وَتَابِعِيْهِمْ بِإِحْسَانٍ إِلَى يَوْمِ الدِّيْن سُبْحَانَ رَبِّك رَبِّ العِزَّةِ عَمَّا يَصِفُوْنَ، وَسَلَامٌ عَلَى المُرْسَلِيْنَ، وَالحَمْدُ لِلَّهِ رَبِّ العَالَمِيْنَ",
    translation:
